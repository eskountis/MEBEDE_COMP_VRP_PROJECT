...
def implement(depot, service_locations, time_matrix, m):
    ...
    while len(available_nodes) > 0:
        ...
        route_order = 0
        for route in routes:
            if min_demand + route.demand > m.truck_capacity:  # check if the current route can fit any additional nodes
                continue
            for i in range(1, len(route.nodes)):
                best_opt = check_for_less_time(i, route.nodes, route.demand, available_nodes, curr_best_time, time_matrix, m.truck_capacity)
